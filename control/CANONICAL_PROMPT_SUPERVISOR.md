...
- NORMAL SERIALIZED CONTINUATION: current ACTIVE_OWNER works continuously. At legal close obtain an external server close reference from the active CLOCK_SOURCE. Under GitHub clock use PRE_CLOSE.created_at; under AUTOMATION_SERVER_CLOCK use an immediately preceding live automation read's server updated_at only as the scheduling reference, then compute NEXT=reference+120s. Create protocol-complete scheduler intent BEFORE mutation; update THIS SAME canonical once with complete recurring VEVENT; independently live-read exact same ID/enabled/exact_schedule/RRULE/exact DTSTART. Under AUTOMATION_SERVER_CLOCK, the scheduler update response updated_at is END and WORKED=END-START. Next invocation proves WAKE_OK/WORK_OK.
